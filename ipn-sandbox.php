...
if (strcmp ($res, "VERIFIED") == 0 || true) {
	...
	if($receiver_email != PAYPAL_BUSINES_EMAIL && $business != PAYPAL_BUSINES_EMAIL) {
		pp_ipn_log_n_die('Bad reciever email expcted ('.PAYPAL_BUSINES_EMAIL.') recieved (receiver_email='.$reciever_email.', business='.$business.')');
	}
	...
	try {
	        ...
	        foreach($result as $r) {
		        ...
		        if(!empty($r['tx_unit_price']) && $r['tx_unit_price'] != $payment_amount/$quantity) {
			        pp_ipn_log_n_die('Payment amount not valid. Expected ('.$r['tx_unit_price'].') recieved ('.$payment_amount.')');
		        }
	        }
	    } catch(PDOExecption $e) { 
	        $dbh->rollback(); 
